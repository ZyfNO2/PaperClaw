...
def test_collection_order_disabled_sources_and_candidate_collision() -> None:
    ...
    assert [item.candidate_id for item in registry.collect(_request())] == ["high", "low"]
    assert disabled.calls == 0

    collision = ContextSourceRegistry()
    collision.register("one", StaticSource(_candidate("same", "first")), kind="custom")
    collision.register("two", StaticSource(_candidate("same", "second")), kind="custom")
...
def test_executor_freezes_registry_and_routes_candidates_through_orchestrator(
    ...
    assert payload["context_source_count"] == 1
    assert "cobalt-42" not in str(payload)

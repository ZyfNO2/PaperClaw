...
9. Keep legacy/direct Coordinator behavior compatible unless a judge model factory is supplied.
...
`Coordinator` accepts an optional `judge_model_factory`. The factory is separate from the Worker execution `model_factory` and may return a different provider/model. Existing direct Coordinator callers remain compatible when it is omitted.

CLI composition uses a dedicated judge model instance and supports optional environment overrides:
...
Each value falls back to the corresponding execution-provider setting when not supplied.
...
Total semantic judge calls are capped at two per Worker completion proposal.
...
- reserve at most two semantic judge calls in Coordinator scheduling.

### Phase C — composition

- CLI: separate judge model factory with environment overrides;
- Desktop: separate model instance using the current provider configuration;
...
- judge model construction is independently configurable;
...
- read-only analytical tasks receive semantic acceptance when the judge is configured;
- legacy callers without a judge factory retain existing behavior;

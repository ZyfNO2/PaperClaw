...
- tool failure / failed terminal；
- partial trace；
- manifest-driven regression test。
...
- 随机合法 model/tool lifecycle；
- `validate → inspect → replay → eval` 确定性；
- 随机 secret redaction；
- 随机 sequence monotonicity。
...
- 200 + request ID；
- bearer token 仅在 header；
- Trace payload 二次脱敏；
- 400 / 401 / 429 / 500；
- timeout；
- connection failure；
- collector response body 不进入错误信息。
...
- Eval PASS；
- JSONL round-trip 相等；
- SQLite 读取前后 SHA256 不变；
- task、文件正文和路径不进入 Trace JSONL。

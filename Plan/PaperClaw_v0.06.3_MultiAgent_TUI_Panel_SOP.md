# PaperClaw v0.07.9 MultiAgent TUI Panel SOP
...
→ v0.07.8 TeamViewReducer / TeamViewSnapshot
→ v0.07.9 TeamApp widgets
```
...
Coordinator callback 只把事件投递回 Textual message loop。状态归约继续由 v0.07.8 `TeamViewReducer` 完成，避免 UI 与 MultiAgent 状态机形成第二套实现。
...
PR #16 必须保持 Draft，并堆叠在 v0.07.8 PR #15。PR #15 合并后，再将 #16 更新到最新 `main` 并执行最终 merge-ref CI。

不得自动合并或删除分支。

## 8. 编号说明

本功能原先以 `v0.06.3` 开发。为统一当前版本线，正式功能编号调整为 `v0.07.9`。文件路径与 GitHub head branch ref 暂时保留旧名，仅用于维持 PR #16 历史与 CI 关联，不代表正式版本号。
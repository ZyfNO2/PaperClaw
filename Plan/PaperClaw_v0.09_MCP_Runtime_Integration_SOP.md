...
- MCP Tool 自动复用 `tool.started/tool.completed/tool.failed` 与 SQLite `session_events`；
...
现有 `NodeRegistry` 只接受 letters、digits、`:`、`_`、`-`。MCP 原始名称允许点号，因此 Registry/Agent action 名不能直接使用 `<server>.<tool>`。
...
默认策略是 `DenyAllMCPPermissionPolicy`。首切片提供显式 `AllowListMCPPermissionPolicy`，匹配 normalized descriptor 的 `server_id.tool_name`。策略异常、无效响应和拒绝均 fail-closed。
...
远端 output/error 在进入 `ToolResult` 前使用现有 `TraceRedactor` 删除配置环境值、Bearer 与路径信息，随后才执行 output truncate。当前通用 Tool lifecycle projection不新增 MCP 专用 event schema。

## 9. 测试矩阵

- MCP 与本地 Tool 共存；
- node-safe identity、稳定 hash 与 Registry collision preflight；
...
- 初始 CI run `29516623795`：发现并修复 MCP 点号 Tool name 与 NodeRegistry 不兼容；同时修复既有 Hypothesis Secret 生成值与固定 JSON key 碰撞问题；

...
class MessageDraft:
    ...
    def __post_init__(self) -> None:
        ...
        _json_object(self.headers, "headers")
        _reject_sensitive_fields(self.payload, "payload")
        _reject_sensitive_fields(self.headers, "headers")
...
class ConsumerCursor:
    ...
    def __post_init__(self) -> None:
        ...
        _topic(self.topic)
        if isinstance(self.ack_sequence, bool) or not isinstance(self.ack_sequence, int) or self.ack_sequence < 0:
    ...
    if isinstance(value, Mapping):
        for raw_key, child in value.items():
            ...
            if key in _SENSITIVE_FIELD_NAMES:
                raise ValueError(f"message contains credential-shaped field: {child_path}")

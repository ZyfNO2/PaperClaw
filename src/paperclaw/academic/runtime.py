...
from .contracts import (
    ...
    ParseResult, RetrievalBudget, RetrievalCandidate, RetrievalResult,
)
...
class AcademicRuntime:
    def __init__(self, workspace: Path, project_id: str) -> None:
    ...
    @classmethod
    def for_workspace(cls, workspace: str | Path, project_id: str) -> "AcademicRuntime":
        return cls(Path(workspace).resolve(strict=True), project_id)
    ...
    def parse_paper(self, paper_id: str) -> ParseResult:
        ...
        with self._connect() as db:
            row = db.execute(
                "SELECT payload FROM parse_manifests WHERE fingerprint=?", (fingerprint,)
            ).fetchone()
        if row:
            return self._parse_result(json.loads(row[0]))
        content = self.papers.read_version(self.project_id, paper_id, version.version_id)
        if version.format != "pdf":
            raise ValueError("structured academic parser currently requires PDF")
        result = self._parse_pdf(paper_id, version.version_id, version.sha256, content, fingerprint)
        payload = self._result_dict(result)
        with self._connect() as db:
            db.execute(
                "INSERT INTO parse_manifests VALUES(?,?,?,?,?)",
                (result.manifest_id, paper_id, version.version_id, fingerprint, json.dumps(payload, ensure_ascii=False)),
            )
            db.commit()
        return result
    ...
    def build_index(self) -> IndexGeneration:
        with self._connect() as db:
            rows = db.execute("SELECT payload FROM parse_manifests ORDER BY manifest_id").fetchall()
        objects = [
            item for row in rows for item in self._parse_result(json.loads(row[0])).objects
            if item.text and item.object_type in {"section", "paragraph", "caption", "table", "equation"}
        ]
        corpus_hash = hashlib.sha256(
            "".join(f"{item.object_id}:{item.text}" for item in objects).encode()
        ).hexdigest()
        generation_id = hashlib.sha256(
            f"{corpus_hash}:{_MODEL_FINGERPRINT}".encode()
        ).hexdigest()
        with self._connect() as db:
            existing = db.execute(
                "SELECT generation_id FROM index_generations WHERE generation_id=?", (generation_id,)
            ).fetchone()
            if not existing:
                db.execute("UPDATE index_generations SET active=0")
                db.execute(
                    "INSERT INTO index_generations VALUES(?,?,?,?,?,1)",
                    (generation_id, corpus_hash, _MODEL_FINGERPRINT, "ready", len(objects)),
                )
                db.execute("DELETE FROM academic_index WHERE generation_id=?", (generation_id,))
                for item in objects:
                    db.execute(
                        "INSERT INTO academic_index VALUES(?,?,?,?,?)",
                        (generation_id, item.object_id, item.text, json.dumps(item.locator.to_dict()), _vector_json(item.text)),
                    )
                db.commit()
        return IndexGeneration(generation_id, corpus_hash, _MODEL_FINGERPRINT, "ready", len(objects))
    ...
    def retrieve(
        ...
        with self._connect() as db:
            active = db.execute("SELECT generation_id FROM index_generations WHERE active=1").fetchone()
            if not active:
        ...
        selected = tuple(item for item in candidates[: budget.max_candidates] if item.fused_score > 0)
        top = selected[0].fused_score if selected else 0
        sufficiency = "sufficient" if top >= 0.65 else "partial" if top >= 0.25 else "insufficient"
        reasons = ("matching grounded objects found",) if selected else ("no grounded object matched",)
        return RetrievalResult(query.text, selected, sufficiency, reasons)

    def expand(self, candidate: RetrievalCandidate, *, neighbors: int = 1) -> tuple[AcademicObject, ...]:
        result = self.get_parse(candidate.locator.paper_id, candidate.locator.version_id)
        ordered = list(result.objects)
        index = next(i for i, item in enumerate(ordered) if item.object_id == candidate.locator.object_id)
    ...
    def resolve(self, locator: AcademicLocator) -> AcademicObject:
        ...
        for item in result.objects:
            if item.object_id == locator.object_id and item.locator.source_hash == locator.source_hash:
                return item
    ...
    def save_evidence_bundle(self, result: RetrievalResult) -> dict[str, object]:
        payload = {
            "schema_version": 1, "query": result.query, "sufficiency": result.sufficiency,
            "reasons": list(result.reasons),
            "candidates": [
                {"locator": item.locator.to_dict(), "text": item.text, "score": item.fused_score}
    ...
    def _parse_pdf(self, paper_id, version_id, source_hash, content, fingerprint):
        ...
        objects = []
        order = 0
        doc_locator = AcademicLocator(paper_id, version_id, f"{version_id}:document", 1, "document", source_hash)
        objects.append(AcademicObject(doc_locator.object_id, "document", order, doc_locator))
        headings: list[str] = []
        for page_index, page in enumerate(document):
            ...
            order += 1
            page_locator = AcademicLocator(paper_id, version_id, f"{version_id}:page:{page_number}",
                                           page_number, "page", source_hash,
                                           bounding_box=BoundingBox(0, 0, page.rect.width, page.rect.height))
            objects.append(AcademicObject(page_locator.object_id, "page", order, page_locator, asset_hash=asset_hash))
            for block_index, block in enumerate(page.get_text("blocks", sort=True)):
                text = str(block[4]).strip()
                if not text:
                    continue
                bbox = BoundingBox(float(block[0]), float(block[1]), float(block[2]), float(block[3]))
                is_heading = len(text) < 160 and (block_index == 0 or re.match(r"^\d+(\.\d+)*\s+\S+", text))
                ...
                locator = AcademicLocator(paper_id, version_id, object_id, page_number,
                                           object_type, source_hash, tuple(headings), bbox)
                objects.append(AcademicObject(object_id, object_type, order, locator, text=text))
            for drawing_index, drawing in enumerate(page.get_drawings()):
                ...
                locator = AcademicLocator(
                    paper_id, version_id, object_id, page_number, "figure", source_hash,
                    tuple(headings), BoundingBox(rect.x0, rect.y0, rect.x1, rect.y1),
                )
                objects.append(
                    AcademicObject(object_id, "figure", order, locator, asset_hash=clip_hash)
                )
            try:
            ...
            for table_index, table in enumerate(tables):
                ...
                locator = AcademicLocator(
                    paper_id, version_id, object_id, page_number, "table", source_hash,
                    tuple(headings), BoundingBox(*map(float, bbox)),
                )
                table_text = "\n".join(
                    " | ".join(str(cell or "") for cell in row)
                    for row in table.extract()
                )
                objects.append(AcademicObject(object_id, "table", order, locator, text=table_text))
        document.close()
        return ParseResult(f"parse-{fingerprint}", paper_id, version_id, _PARSER, _PARSER_VERSION,
                           fingerprint, "ready", len([o for o in objects if o.object_type == "page"]),
                           tuple(objects))
    ...
    def _result_dict(self, result):
        return {**asdict(result), "objects": [item.to_dict() for item in result.objects]}

    def _parse_result(self, payload):
        objects = tuple(
            AcademicObject(item["object_id"], item["object_type"], item["reading_order"],
                           _locator(item["locator"]), item.get("text"), item.get("asset_hash"),
                           item.get("provenance", "extracted"))
            for item in payload["objects"]
        )
        return ParseResult(payload["manifest_id"], payload["paper_id"], payload["version_id"],
                           payload["parser_name"], payload["parser_version"], payload["parser_fingerprint"],
                           payload["status"], payload["page_count"], objects, tuple(payload.get("warnings", ())))


def _locator(value):
    bbox = BoundingBox(**value["bounding_box"]) if value.get("bounding_box") else None
    return AcademicLocator(value["paper_id"], value["version_id"], value["object_id"],
                           value["page_number"], value["object_type"], value["source_hash"],
                           tuple(value.get("section_path", ())), bbox)

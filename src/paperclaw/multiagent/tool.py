...
The parent agent supplies an explicit task DAG. Each Worker receives a fresh
AgentRuntime state, a scoped tool registry, and no parent conversation history.
...
class SubagentTaskTool:
    """Delegate a bounded task DAG to isolated Workers and return summaries only."""
    ...
    description = (
        ...
        "only structured summaries to the parent."
    )
    ...
    def validate(self, arguments: dict[str, Any]) -> None:
        _parse_request(arguments)

    def execute(self, arguments: dict[str, Any], context: ToolContext) -> ToolResult:
        goal, tasks, budget = _parse_request(arguments)
        stop_token = context.stop_token
        if stop_token is not None and stop_token.is_cancelled:
            return ToolResult(False, "subagent delegation cancelled", "cancelled")
        ...
        def run_team() -> None:
            ...
            except Exception as exc:  # defensive boundary around background Workers
                holder["error"] = exc
...
def _parse_request(
    ...
    max_agents = _bounded_int(arguments.get("max_agents", 2), "max_agents", 1, _MAX_AGENTS)
    total_steps = sum(task.max_steps for task in tasks)
    max_timeout = max(task.timeout_seconds for task in tasks)
    budget = TeamBudget(
        max_agents=min(max_agents, len(tasks)),
        max_total_steps=min(100, total_steps),
        max_total_model_calls=min(120, total_steps + (2 * len(tasks))),
...
def _parse_task(index: int, value: Any) -> AgentTask:
    ...
    criteria = _string_list(item.get("acceptance_criteria"), "acceptance_criteria", required=True)
    allowed_paths = _string_list(item.get("allowed_paths", ["."]), "allowed_paths")
    writable_paths = _string_list(item.get("writable_paths", []), "writable_paths")
    ...
    if unknown_tools:
        raise ToolValidationError(
            "subagents cannot use unknown or recursive tools: " + ", ".join(unknown_tools)
        )
    dependencies = _string_list(item.get("dependencies", []), "dependencies")
    max_steps = _bounded_int(item.get("max_steps", 8), "max_steps", 1, _MAX_TASK_STEPS)

...
- stable `mcp.<server_id>.<tool_name>` registration into `ToolRegistry`;
- atomic discovery/registration with collision preflight;
...
- structured MCP Tool failures that do not crash the local Runtime;
- local Tool survival when MCP connect/discovery/call fails;
- unit tests, real local stdio Fake Server integration, and Runtime/Trace integration tests.

## Architecture decisions

1. No MCP-specific Agent, QueryEngine, Budget or Trace path was introduced.
2. Registry integration is explicit; capability selection remains out of scope.
3. Registration failure is returned as `MCPRegistrationResult`, leaving existing local Tools unchanged.
4. The Permission boundary defaults to deny and is evaluated at execution time, not cached at discovery time.
5. Synchronous protocol calls run behind a cancellation-aware daemon worker because the Phase A session remains single-flight and synchronous.
6. Timeout/cancellation closes the connection to prevent late responses from contaminating subsequent requests.
7. Environment values used to launch a Server become exact redaction inputs but never enter durable metadata.

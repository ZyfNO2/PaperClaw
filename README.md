...
- v0.06：Claw TUI MVP 的实现与离线 CI，真实 Windows Terminal / Live Provider 交互验收仍待完成。
...
- 窄终端单列退化；
- TUI 不导入具体 Tool、Repository 或 SQLite。

v0.06 不包含 token streaming、Shell stream/background、Permission Dialog、Session Picker、Inspector、MultiAgent view、Web UI/API 或强制进程取消。
